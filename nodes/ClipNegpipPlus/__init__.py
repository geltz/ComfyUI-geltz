...
def negpip_attn(q, k, v, extra_options):
    """Extract keys from even positions, values from odd positions."""
    new_k = k[:, 0::2]
    new_v = v[:, 1::2]
...
def encode_token_weights_negpip_plus(
    ...
    for x in token_weight_pairs:
        ...
        has_weights = has_weights or any(a[1] != 1.0 for a in x)
    ...
    first_pooled = pooled[0:1].to(model_management.intermediate_device()) if pooled is not None else None
        ...
        if has_weights:
            ...
            for i in range(len(zk)):
                for j in range(len(zk[i])):
                    ...
                    if weight < 0:
                        # Suppress negative: reduce key magnitude, invert value with damping
                        weight_abs = abs(weight)
                        zk[i][j] = z_empty[j] + (zk[i][j] - z_empty[j]) * (weight_abs / alpha)
                        zv[i][j] = z_empty[j] - (zv[i][j] - z_empty[j]) * weight_abs
                    else:
    ...
    if len(o) > 2:
        ...
        for k in o[2]:
            v = o[2][k]
            if k == "attention_mask":
                v = v[:sections].flatten().unsqueeze(0).to(model_management.intermediate_device())
            extra[k] = v
        r = r + (extra,)
...
class CLIPNegPipPlus(ComfyNodeABC):
    ...
    def patch(self, model: ModelPatcher, clip: CLIP, alpha: float, delta: float, sigma: float):
        ...
        if not m.model_options.get(NEGPIP_PLUS_OPTION):
            ...
            m.model_options[NEGPIP_PLUS_OPTION] = True
            c.patcher.model_options[NEGPIP_PLUS_OPTION] = True
