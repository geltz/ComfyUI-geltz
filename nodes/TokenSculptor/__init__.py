...
def _get_embeddings(clip_model):
    return clip_model.transformer.text_model.embeddings.token_embedding.weight.to(
...
def blended_kl_bounded_step(w0, Wn, sched, att, kappa=0.03):
    ...
    for k_i, tau_i in zip(ks, taus):
        ...
        w_star_i = trust_region_step(
            w0, Wn, idx_i, alpha_i, theta_max_deg=sched["theta_max_deg"], step_scale=step_scale
        )
        d_i = w_star_i - w0
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "sculpt_strength": ("FLOAT", {"default": 0.5, "min": 0.0, "max": 1.0, "step": 0.01}),
            }
        }
